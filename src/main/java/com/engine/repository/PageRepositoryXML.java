...
public class PageRepositoryXML {
	 ...
	 */
	private String[] blackListFiles = { ".DS_Store" };
		...
		try {
			...
			if (folder.listFiles().length == 0) {
				...
				for (File file : files) {
					...
					if (file.isFile() && (!Arrays.stream(blackListFiles).anyMatch(file.getName()::equals))) {
	 ...
	 * 
	 * public List<Document> loadPages(String xmlStringPath) { Grid page = null;
	 * List<Grid> pages = new ArrayList<Grid>();
	 * 
	 * try {
	 * 
	 * System.out.println("Percorso " + xmlStringPath + "..."); File folder = new
	 * File(xmlStringPath);
	 * 
	 * if (!folder.exists() || !folder.isDirectory() || folder==null) return null;
	 * 
	 * File[] files = folder.listFiles(); for (File file : files){
	 * 
	 * System.out.println("Accedo al file " + xmlStringPath + file.getName());
	 * Unmarshaller unmarshaller =
	 * JAXBContext.newInstance(Grid.class).createUnmarshaller(); page = (Grid)
	 * unmarshaller.unmarshal(file); System.out.println("Page "+ page
	 * +" caricato!"); pages.add(page); }
	 * 
